...
import com.agnext.qualixfarmer.network.Response.CropData
...
class AddFarmActivity : BaseActivity(), View.OnClickListener, AdapterView.OnItemSelectedListener,
    ...
    fun initView() {
        ...
        viewModel.getCrop(userToken(this))
    }
    ...
    private fun addFarm() {
        if (tvFarmCode.text!!.isNotEmpty() && tvFarmName.text!!.isNotEmpty() && tvDistrict.text!!.isNotEmpty() && tvArea.text!!.isNotEmpty() && tvAddress.text!!.isNotEmpty()) {
            ...
            data["farmName"] = tvFarmName.text.toString()
            data["district"] = tvDistrict.text.toString()
            data["area"] = tvArea.text.toString()
            data["address"] = tvAddress.text.toString()
            data["farmerId"]=SessionClass(this).getUserId()

            if (spinnerCrop != null && spinnerCrop.selectedItem != null) {
                data["cropId"] = viewModel.getCropList.value!!.cropData!![spinnerCrop.selectedItemPosition].cropId.toString()
            }
            ...
            if(points!=null)
            {
                data["farmIndics"]=points
            }

            spinnerCrop.selectedItem
            viewModel.addFarm(userToken(this), data)
    ...
    override fun onNothingSelected(p0: AdapterView<*>?) {
        ...
        when (parent) {
            spinnerCrop -> {
                ...
                viewModel.getCropVariety(userToken(this), crops[pos].cropId.toString())
            }
        }
    }
